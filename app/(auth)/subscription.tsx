...
export default function SubscriptionScreen() {
  ...
  const handlePurchaseRcPackage = async (pkg: PurchasesPackage) => {
    ...
    if (success) {
      showSuccessMessage("Subscription plan selected successfully!");
      const refreshedUser = await getCurrentUser();
      if (refreshedUser) {
        setUser(refreshedUser, tokens);
      }
      await refreshSubscription();
      router.replace((refreshedUser && hasCompletedOnboarding(refreshedUser) ? "/(tabs)/home" : "/(auth)/setup") as any);
    }
  };
  ...
  const handleContinueWithCurrentState = async () => {
    if (isPro) {
      router.replace((user && hasCompletedOnboarding(user) ? "/(tabs)/home" : "/(auth)/setup") as any);
    } else {
      showErrorMessage("Please select a subscription plan to continue.");
    }
  };

  return (
    <SafeAreaView style={styles.container} edges={["bottom"]}>
      <Header title="Choose Your Plan" showBack={false} />


      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {/* Active Member Status Card if already subscribed */}
        {isPro ? (
          <View style={[styles.activeCard, isCanceledButActive && { borderColor: "#EAB308" }]}>
            <View style={styles.activeHeaderRow}>
              ...
              <Text style={styles.activeTitle}>
                {isCanceledButActive ? "Subscription Canceled" : "Active Premium Member"}
              </Text>
            </View>
        ...
        {/* Primary Continue / Action Button if active */}
        {isPro ? (
          <TouchableOpacity
            style={[styles.upgradeButton, { marginTop: verticalScale(12) }]}
            onPress={handleContinueWithCurrentState}
          >
            <Text style={styles.upgradeButtonText}>Continue to App</Text>
          </TouchableOpacity>
        ) : null}
